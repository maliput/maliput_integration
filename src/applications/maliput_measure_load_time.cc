  ...
  for (int i = 0; i < FLAGS_iterations; i++) {
    ...
    times.push_back(
        MeasureLoadTime(maliput_implementation,
                        ...
                        {FLAGS_yaml_file},
                        {FLAGS_xodr_file_path, FLAGS_linear_tolerance, GetMaxLinearToleranceFlag(), FLAGS_build_policy,
                         FLAGS_num_threads, FLAGS_simplification_policy, FLAGS_standard_strictness_policy,
                         FLAGS_omit_nondrivable_lanes, FLAGS_road_rule_book_file, FLAGS_traffic_light_book_file,
